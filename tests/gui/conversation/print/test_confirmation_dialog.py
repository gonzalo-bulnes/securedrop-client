...
class TestConfirmationDialog(unittest.TestCase):
    ...
    def assertDialogReflectsPrinterStatus(self, dialog: conversation.PrintConfirmationDialog, status: Printer.Status):
        self.assertTrue(expected_message[status] in dialog.text(), f"expected to find \"{expected_message[status]}\" in dialog text to reflect printer status {status}")
        if status == Printer.StatusReady:
            self.assertTrue(self.dialog.continue_button.isEnabled(), f"expected print button to be enabled when status is {status}, was disabled")
        else:
            self.assertTrue(not self.dialog.continue_button.isEnabled(), f"expected print button to be disabled when status is {status}, was enabled")

    def setUp(self):
        controller = MagicMock(spec=Controller)
        export_service = MagicMock(spec=export.Service)
    ...
    def test_displays_which_file_will_be_printed(self):
        ...
        self.assertTrue(file_name in dialog.text(), f"expected to find \"{file_name}\" in \"{dialog.text()}\"")
    ...
    def test_displays_sanitized_file_name_not_dangerous_file_name(self):  # TODO: define "dangerous"
        ...
        printer = MagicMock(spec=Printer)
        dialog = conversation.PrintConfirmationDialog(printer, file_name)
        self.assertTrue(safe_file_name in dialog.text(), f"expected to find \"{safe_file_name}\" in \"{dialog.text()}\"")
        self.assertTrue(dangerous_file_name not in dialog.text(), f"expected NOT to find \"{dangerous_file_name}\" in \"{dialog.text()}\"")
